...
```
configs/
  safety_default.yaml      標準設定（0-100m, vis≠NONE）
  safety_near_strict.yaml  近距離厳格（0-60m, FULL/MOST のみ、歩行者2倍重み）
  safety_full_range.yaml   広域（0-150m, vis≠NONE）
  rematch_default.yaml     標準再マッチング（2m, strict ラベル）
  rematch_strict.yaml      厳格再マッチング（per-class閾値, conf≥0.5, vis≠NONE）
  rematch_grouped.yaml     グループ再マッチング（motorbike+bicycle統合）

rematch/                   GT-EST 再マッチング パッケージ
  config.py                RematchConfig dataclass（YAML I/O + argparse）
  match.py                 Hungarian / greedy マッチングロジック
  __init__.py

safety_metrics/            安全指標計算 パッケージ
  config.py                SafetyConfig dataclass（YAML I/O + argparse）
  compute.py               純粋計算関数（I/O なし）
  __init__.py

run_rematch.py             再マッチング CLI
run_safety_metrics.py      安全指標 CLI
...
```bash
# インストール（省略可 — clone して直接 python でも動く）
pip install .

# 安全指標を計算
python run_safety_metrics.py --config configs/safety_default.yaml \
    --csv path/to/current.csv

# 距離だけ変えて比較
python run_safety_metrics.py --config configs/safety_default.yaml \
    --csv path/to/current.csv \
    --dist-max 60 --label v4.4.0_60m --output-dir output/safety_metrics/60m

# 結果を横並び比較
python run_safety_metrics.py --compare \
    output/safety_metrics/default/summary.json \
    output/safety_metrics/60m/summary.json

# GT-EST を再マッチングしてから安全指標を計算
python run_rematch.py --config configs/rematch_default.yaml \
    --csv path/to/current.csv
python run_safety_metrics.py --config configs/safety_default.yaml \
    --csv output/rematch/default/rematched.csv --label rematch_default
```
